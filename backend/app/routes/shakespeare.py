...
from ..schemas.request_models import (
    ParaphraseRequestModel,
    T5SummaryRequestModel,
)

shakespeare_router = APIRouter(prefix="/api")
...
async def paraphrase(
    request_data: ParaphraseRequestModel,
    user: User | None = Depends(get_current_user),
    db: Session = Depends(get_session),
):
    summary: Summary = Summary()
    summarized_data = summary.generate_summary(
        request_data.text, int(len(request_data.text.split(".")) / 2)
    )
    print(user)
    if user:
        create_shorten_request(db, summarized_data, request_data.text, user, "math")

    return {"text": request_data.text, "summarized": summarized_data, "type": "math"}


@shakespeare_router.post("/t5_paraphrase", tags=["ml_models_endpoint"])
async def t5_summary(
    request_data: T5SummaryRequestModel,
    user: User | None = Depends(get_current_user),
    db: Session = Depends(get_session),
):
    model: T5Summary = models["t5"]
    text = model.summarize(
        request_data.text, request_data.min_length, request_data.max_length
    )[0]["summary_text"]

    if user:
        create_shorten_request(db, text, request_data.text, user, "nn")

    return {"text": request_data.text, "summarized": text, "type": "t5"}
...
async def bert_summary(request_data: T5SummaryRequestModel):
    ...
    return {"text": request_data.text, "summarized": text, "type": "bert"}
